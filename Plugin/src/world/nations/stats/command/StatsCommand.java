...
public class StatsCommand extends MassiveCommand {
	...
	public void perform() {
		...
		UPlayer fplayer = UPlayer.get(this.arg(0));
		
		if (!fplayer.hasFaction() && this.arg(0).equalsIgnoreCase(player.getName())) {
			player.sendMessage("§cCe joueur n'a pas de pays !");
			return;
		}
		...
		player.sendMessage(Utils.color("&eAlliance&7: &d" + (data.isAlly() ? "Alliés" : data.isAxe() ? "Axe" : data.isUrss() ? "URSS" : "Neutre")));
		...
		player.sendMessage(Utils.color("&eScore Zone&7: &c" + data.getPoints()));
		return;
	}
